...
public class VoteDAOJdbcImpl implements VoteDAO{
	...
	public void add(Vote vote) {
                        ...
                        Object[] params = new Object[]{ vote.getVoter().getVoterId(), vote.getInnovation().getInnoId(), vote.isLegit() };
                        int rows = this.jdbc.update(query, params);
                        if(rows < 1){ log.warn("Could not add vote for innovation=%d", vote.getInnovation().getInnoId()); }
                        else{ log.info(String.format("New vote was added for innovation=%d", vote.getInnovation().getInnoId())); }
	}
